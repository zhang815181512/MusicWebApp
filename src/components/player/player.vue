<template>
  <div class="player" v-show="playlist.length>0">
    ...
    <audio ref="audio" :src="currentSong.url"
           @play="ready" @error="error" @timeupdate="updateTime" @ended="end"
    ></audio>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      changeMode() {
        ...
        this.resetCurrentIndex(list) // 问题是打乱顺序但不影响当前的播放
        this.setPlaylist(list)
      },
      resetCurrentIndex(list) {
        let index = list.findIndex((item) => {
          return item.id === this.currentIndex.id
        })
        this.setCurrentIndex(index)
      },
      getLyric() {
      ...
      middleTouchMove(e) {
        ...
        const offsetWidth = Math.min(0, Math.max(-window.innerWidth, left + deltaX)) // 移动宽度
      ...
      enter(el, done) {
        /*
        * 此处js动态创建css3动画  create-keyframe-animation
        * */
